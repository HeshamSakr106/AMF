pipeline {
    ...
    stages {
        ...
        stage('Scan Image for Common Vulnerabilities and Exposures') {
            steps {
                sh 'docker run --rm -v /var/run/docker.sock:/var/run/docker.sock -v C:/Users/BADR/AppData/Local/Docker/wsl:/images aquasec/trivy image abodiaa/5g-amf:latest --output /images/trivy-report.json'
            }
        }
    ...
    post {
        always {
            ...
            sh 'docker rmi abodiaa/5g-amf:latest'
            sh 'docker rmi abodiaa/amf-base:latest'
        }
    }
}
